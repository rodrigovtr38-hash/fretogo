...
export const ALLOWED_TRANSITIONS: Record<TripState, TripState[]> = {
  ...
  [TripState.COLETANDO]: [TripState.EM_TRANSPORTE, TripState.REDISPATCH], // Não pode cancelar durante o carregamento pelo App (só via suporte)
  [TripState.EM_TRANSPORTE]: [TripState.FINALIZANDO, TripState.REDISPATCH], // Redispatch aqui é guincho/sinistro
  [TripState.FINALIZANDO]: [TripState.ENTREGUE],
  [TripState.ENTREGUE]: [], // Estado Final (Repasse é feito no Admin, não muda status logístico)
  [TripState.CANCELADO]: [], // Estado Final
  [TripState.EXPIRADO]: [], // Estado Final
  [TripState.REDISPATCH]: [TripState.DISPONIVEL, TripState.CANCELADO] // Voltou pro radar
};
