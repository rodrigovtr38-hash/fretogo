import { Clock, Zap, Package, MapPin, Truck, Layers, Info } from 'lucide-react';
...
interface OrderData {
  ...
  valorPedagio?: number; // 🔥 Campo de pedágio injetado
  distancia?: number;
...
export default function OfertaModal({
  ...
  const temPedagio = typeof ofertaFrete.valorPedagio === 'number' && ofertaFrete.valorPedagio > 0;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 p-4 backdrop-blur-sm animate-in zoom-in-95 duration-200">
      <div className="relative w-full max-w-md overflow-hidden rounded-[2.5rem] border border-green-500/30 bg-slate-900 shadow-[0_20px_60px_rgba(34,197,94,0.15)] flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar flex-1">
          ...
          {/* GANHOS OPERACIONAIS (MARKUP REVERSO APLICADO) */}
          <div className="rounded-3xl border border-white/5 bg-slate-950/60 p-5 text-center">
            <p className="mb-1 text-[10px] font-black uppercase tracking-widest text-slate-500">Ganhos Operacionais Livres</p>
            ...
            {/* 🔥 ALERTA TRANSPARENTE DE PEDÁGIO */}
            {temPedagio && (
              <div className="mt-3 flex items-center justify-center gap-1.5 bg-blue-500/10 text-blue-400 p-2 rounded-xl border border-blue-500/20">
                <Info size={14} />
                <p className="text-[10px] font-bold uppercase tracking-widest">
                  + R$ {ofertaFrete.valorPedagio?.toFixed(2).replace('.', ',')} de Vale-Pedágio Antecipado
                </p>
              </div>
            )}
