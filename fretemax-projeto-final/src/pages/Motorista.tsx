...
export default function Motorista() {
  ...
  // 7. CLEANUP ABSOLUTO - Evita memory leaks de áudio e listeners órfãos
  useEffect(() => {
    return () => {
      setToast(null);
  ...
  useEffect(() => {
    ...
    const messages = [
      'Buscando cargas...',
      'Carga priorizada para sua rota...',
      'Retorno compatível encontrado...',
      'Corrida inteligente disponível...'
    ];
    let i = 0;
  ...
  // UPDATE OPERACIONAL: Heartbeat do GPS e FAILSAFE (GPS Lost -> Offline)
  useEffect(() => {
    if (user && driverData?.status === 'aprovado' && isOnline) {
      const updatePosition = async (pos: GeolocationPosition) => {
        ...
        if (now - lastGpsUpdate.current >= GPS_HEARTBEAT_MS) {
          ...
          try {
            ...
            // Heartbeat Transacional para o Cliente em rota (Permite o Watchdog cliente-side)
            if (activeFrete?.id) {
              await updateDoc(doc(db, 'fretes', activeFrete.id), { motoristaLat: pos.coords.latitude, motoristaLng: pos.coords.longitude, motoristaLastSeen: serverTimestamp() }).catch(() => {});
            }
          } catch (e) {}
        }
      };
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        updatePosition, 
        async (err) => {
          // 5. FAILSAFE GPS - Motorista desconectado se GPS falhar
          showToast("Sinal de GPS perdido. Você está offline por segurança.", "error");
          setIsOnline(false);
          try { await deleteDoc(doc(db, 'motoristas_online', user.uid)); } catch(e){}
          
          if (activeFrete?.id) {
            try {
              const fn = httpsCallable(getFunctions(), 'triggerRedispatch');
              await fn({ freteId: activeFrete.id, motoristaId: user.uid, reason: 'gps_lost' });
            } catch(e) {}
          }
        }, 
  ...
  // SISTEMA DE OFERTAS COM FAILSAFES E LOCK
  useEffect(() => {
  ...
  }, [user, isOnline, activeFrete, stopAudio]);
  ...
  // TRANSAÇÃO DE ACEITE BLINDADA (Mutex)
  const handleAceitar = async () => {
    if (!exibindoOferta || !ofertaFrete || !user || actionLock.current) return;
    ...
    try {
      ...
      await runTransaction(db, async (t) => {
        ...
        if (data.status !== AppTripState.DISPONIVEL || data.motoristaId) throw new Error("JA_ACEITO");
    ...
    } finally { setExibindoOferta(false); setOfertaFrete(null); currentOfferId.current = null; actionLock.current = false; }
  };

  const handleRecusar = async (targetId?: string) => {
    if (!exibindoOferta && !targetId) return;
    ...
    setExibindoOferta(false); setOfertaFrete(null); currentOfferId.current = null; actionLock.current = false;
  };
  ...
  // 8. OTIMIZAÇÃO EXTREMA DE SNAPSHOTS (limit: 1)
  useEffect(() => {
    let unsubCad: any; let unsubFretes: any;
    const unsubscribeAuth = auth.onAuthStateChanged((u) => {
      if (u) {
        setUser({ uid: u.uid, email: u.email }); setLoading(true);
        ...
        unsubFretes = onSnapshot(
          query(
            ...
            where('motoristaId', '==', u.uid), 
            where('status', 'in', [AppTripState.ACEITO, AppTripState.INDO_COLETA, AppTripState.COLETANDO, AppTripState.EM_TRANSPORTE]),
            limit(1)
          ), 
          (s) => {
            if (!s.empty) {
              setActiveFrete({ id: s.docs[0].id, ...s.docs[0].data() } as OrderData);
            } else {
              setActiveFrete(null);
            }
            setLoading(false);
  ...
  const toggleStatus = async () => {
    ...
    if (isOnline) {
      ...
      setLoadingMessage('Sincronizando GPS de Alta Precisão...');
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          try {
            ...
            showToast("Falha na sincronização de segurança.", "error");
          } finally {
            actionLock.current = false;
          }
        },
        (err) => {
          showToast("Sinal de GPS obrigatório. Ative a alta precisão.", "error");
          actionLock.current = false;
        },
  ...
  // 2. MÁQUINA DE ESTADO COM GUARDS E TRANSACTIONS (Bloqueia estados fantasmas)
  const updateStatusFrete = async (nextStatus: AppTripState) => {
    if (!activeFrete || !activeFrete.id || !user || actionLock.current) return;

    if (!canTransition(activeFrete.status, nextStatus)) {
      showToast("Transição de status inválida no momento.", "error");
      return;
    }

    actionLock.current = true;
    try {
      if (nextStatus === AppTripState.ENTREGUE) {
        if (!comprovante) { showToast("Anexe a foto do canhoto.", "warning"); actionLock.current = false; return; }
        setLoading(true);
        ...
        await runTransaction(db, async (t) => {
           ...
           const snap = await t.get(freteRef);
           if (!snap.exists() || !canTransition(snap.data().status as AppTripState, nextStatus)) {
              throw new Error("INVALID_STATE");
           }
           t.update(freteRef, { status: nextStatus, comprovanteUrl: url, entregueEm: serverTimestamp() });
        });
        ...
        await runTransaction(db, async (t) => {
           ...
           if (nextStatus === AppTripState.COLETANDO) updateData.coletaEm = serverTimestamp();
           if (nextStatus === AppTripState.EM_TRANSPORTE) updateData.transporteEm = serverTimestamp();
           t.update(freteRef, updateData);
        });
      }
    } catch(e) {
      showToast("Erro ao atualizar status. Rota não autorizada.", "error");
      setLoading(false);
    } finally {
      actionLock.current = false;
    }
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-[#020617] text-slate-200 font-sans selection:bg-cyan-500/30">
      ...
      {/* MAIN */}
      <main className="relative z-10 flex-1 mx-auto flex w-full max-w-[1400px] items-center flex-col justify-center px-4 py-8 pb-32 sm:px-6 lg:px-8">
        ...
        {exibindoOferta && ofertaFrete && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 p-5 backdrop-blur-md animate-in zoom-in-95 duration-200">
            <div className="relative w-full max-w-md overflow-hidden rounded-[2.5rem] border border-green-500/30 bg-slate-900 p-8 text-center shadow-[0_20px_60px_rgba(34,197,94,0.2)]">
              <div className="absolute left-0 top-0 h-1.5 bg-green-500 transition-all duration-1000" style={{ width: `${(tempoRestante / 15) * 100}%` }} />
              ...
              <div className="flex gap-4">
                ...
                <button onClick={handleAceitar} className="flex-1 rounded-[1.25rem] bg-green-500 px-6 py-5 text-[15px] font-black uppercase italic tracking-widest text-slate-950 shadow-[0_15px_30px_rgba(34,197,94,0.3)] transition-all hover:scale-[1.02] active:scale-95">Aceitar Carga</button>
              </div>
            </div>
          </div>
        )}
        ...
        {!user ? (
          <div className="mx-auto mt-10 flex min-h-[60vh] w-full max-w-md flex-col items-center justify-center">
            <div className="w-full rounded-[3rem] border border-white/10 bg-slate-900/60 p-8 md:p-8 text-center shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-xl">
          ...
          <div className="mx-auto w-full max-w-2xl rounded-[2.5rem] border border-white/10 bg-slate-900/60 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-md">
            <div className="mb-8 flex items-center justify-center gap-4 border-b border-slate-800 pb-6">
              ...
              <h2 className="leading-none text-2xl font-black uppercase italic tracking-tight text-white">Seu Veículo,
                <span className="text-cyan-400">Seu Dinheiro.</span></h2>
            </div>
          ...
          <div className="mx-auto w-full max-w-lg rounded-[3rem] border border-cyan-500/30 bg-slate-900/80 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-xl md:p-10">
            ...
            <div className="space-y-4">
              <button onClick={() => openMaps(activeFrete.status === AppTripState.ACEITO ? activeFrete.origemLat! : activeFrete.destinoLat!, activeFrete.status === AppTripState.ACEITO ? activeFrete.origemLng! : activeFrete.destinoLng!)} className="flex w-full min-h-[72px] items-center justify-center gap-3 rounded-[1.5rem] border border-white/10 bg-slate-950 px-6 py-5 text-sm font-black uppercase tracking-[0.2em] text-white shadow-inner transition-colors hover:bg-slate-900">
                <Navigation size={20}/> Abrir no GPS
              </button>
              
              {activeFrete.status === AppTripState.ACEITO && <button onClick={() => updateStatusFrete(AppTripState.COLETANDO)} className="flex w-full min-h-[72px] items-center justify-center gap-3 rounded-[1.5rem] bg-cyan-500 px-6 py-5 text-[15px] font-black uppercase italic tracking-[0.2em] text-slate-950 shadow-[0_15px_40px_rgba(6,182,212,0.35)] transition-all hover:scale-[1.02] active:scale-95"><MapPin className="h-6 w-6"/> Cheguei na Coleta</button>}
              {activeFrete.status === AppTripState.COLETANDO && <button onClick={() => updateStatusFrete(AppTripState.EM_TRANSPORTE)} className="flex w-full min-h-[72px] items-center justify-center gap-3 rounded-[1.5rem] bg-yellow-400 px-6 py-5 text-[15px] font-black uppercase italic tracking-[0.2em] text-slate-950 shadow-[0_15px_40px_rgba(250,204,21,0.35)] transition-all hover:scale-[1.02] active:scale-95"><Truck className="h-6 w-6"/> Carga Embarcada</button>}
              {activeFrete.status === AppTripState.EM_TRANSPORTE && (
          ...
          <div className="mx-auto mt-10 flex w-full max-w-[1600px] flex-col items-center text-center">
            ...
            {!isOnline && deferredPrompt && (
              <button onClick={() => { deferredPrompt.prompt(); deferredPrompt.userChoice.then((choice: any) => { if (choice.outcome === 'accepted') setDeferredPrompt(null); }); }} className="mx-auto mt-12 flex items-center justify-center gap-3 rounded-2xl border border-white/10 bg-slate-900 px-8 py-5 text-xs font-black uppercase tracking-widest text-slate-300 transition-colors hover:bg-slate-800">
