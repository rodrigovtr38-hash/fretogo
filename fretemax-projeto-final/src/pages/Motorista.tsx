        ...
        {!user ? (
          ...
          <div className="mx-auto w-full max-w-md rounded-[2.5rem] border border-white/10 bg-slate-900/60 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-md">
            ...
            <div className="space-y-4">
              <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="Nome Completo" onChange={e => setForm({...form, nome: e.target.value})} />
              <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="CPF" onChange={e => setForm({...form, cpf: e.target.value})} />
              <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="Cidade/Estado" onChange={e => setForm({...form, cidadeEstado: e.target.value})} />
              <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="WhatsApp" onChange={e => setForm({...form, whatsapp: e.target.value})} />
              <div className="grid grid-cols-2 gap-4">
                <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold uppercase text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="Placa" onChange={e => setForm({...form, placa: e.target.value})} />
                <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold text-white outline-none transition-all placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="CNH" onChange={e => setForm({...form, cnh: e.target.value})} />
              </div>
              <select className="w-full cursor-pointer appearance-none rounded-2xl border border-white/10 bg-slate-950 p-5 text-sm font-bold text-white outline-none transition-colors focus:border-cyan-500/50" onChange={e => setForm({...form, categoria: e.target.value as VehicleType})}>
          ...
          <div className="mx-auto mt-10 flex w-full max-w-md flex-col items-center text-center">
            ...
            {isOnline ? (
              <div className="w-full animate-in fade-in duration-500">
                ...
                {/* ROTA DE RETORNO REMOVIDA DA TELA INICIAL COMO SOLICITADO */}
              </div>
            ) : (
