...
export default function Cliente() {
  ...
  const handleContratar = async () => {
    ...
    } catch (e) { alert("Erro na contratação."); setLoadingPay(false); }
  };
           ...
           <div className="bg-white rounded-[3rem] p-8 text-center shadow-2xl border-4 border-slate-100 mt-10">
              {['aceito', 'coleta', 'em_transporte'].includes(orderData?.status) ? (
                 ...
                 <div className="py-10 text-center">
                    ...
                    {orderData?.tipoFrete === 'agendado' && <div className="bg-blue-100 text-blue-700 p-4 rounded-2xl mt-4 font-black text-xs uppercase">📅 Agendado para: {new Date(orderData.dataAgendada.seconds * 1000).toLocaleString()}</div>}
                 </div>
              )}
           </div>
        )}
