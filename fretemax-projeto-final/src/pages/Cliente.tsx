...
export default function Cliente() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#020617] text-slate-200 font-sans flex flex-col overflow-x-hidden">
      ...
      <div className="pointer-events-none fixed inset-0 z-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-30 mix-blend-screen">
          <div className="relative flex h-[1000px] w-[1000px] items-center justify-center lg:h-[1200px] lg:w-[1200px]">
            <div className="absolute inset-0 animate-ping rounded-full border border-cyan-500/30 shadow-[inset_0_0_40px_rgba(6,182,212,0.15)]" style={{ animationDuration: '4s' }} />
            <div className="absolute inset-[15%] animate-ping rounded-full border border-cyan-400/20 shadow-[inset_0_0_30px_rgba(6,182,212,0.15)]" style={{ animationDuration: '4s', animationDelay: '1s' }} />
          </div>
        </div>
      </div>
      ...
      <header className="relative z-50 w-full border-b border-white/5 bg-slate-950/80 backdrop-blur-xl">
        <nav className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          <div className="flex items-center gap-4">
            <button onClick={() => { if (step === 'form') window.location.href = '/'; else resetFlow(); }} className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 transition-all hover:bg-white/10 hover:scale-105 active:scale-95">
          ...
          <div className="hidden items-center gap-2 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-5 py-2 md:flex">
      ...
      {/* MAIN CONTAINER DA TELA DO CLIENTE (O Segredo da Centralização) */}
      <main className="relative z-10 flex-1 flex w-full">
        {/* Aqui é a âncora absoluta: container mx-auto inquebrável, com flex-col justify-center para ir pro meio vertical */}
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col justify-center w-full max-w-6xl">
        
          {step === 'form' && (
            <div className="w-full rounded-[2.5rem] border border-white/10 bg-slate-900/60 p-8 shadow-2xl backdrop-blur-xl md:p-12">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="Seu Nome Completo" value={nome} onChange={(e) => setNome(e.target.value)} />
                  <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="WhatsApp (DDD)" value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} />
                </div>
              </div>
              ...
              <div className="mb-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <input className="col-span-2 w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-blue-500/50" placeholder="Rua da Retirada" value={coleta.rua} onChange={e => setColeta({...coleta, rua: e.target.value})} />
                    <input className="col-span-1 w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-blue-500/50" placeholder="Nº" value={coleta.num} onChange={e => setColeta({...coleta, num: e.target.value})} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-blue-500/50" placeholder="Bairro" value={coleta.bairro} onChange={e => setColeta({...coleta, bairro: e.target.value})} />
                    <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-blue-500/50" placeholder="CEP" value={coleta.cep} onChange={e => setColeta({...coleta, cep: e.target.value})} />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <input className="col-span-2 w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-green-500/50" placeholder="Rua da Entrega" value={entrega.rua} onChange={e => setEntrega({...entrega, rua: e.target.value})} />
                    <input className="col-span-1 w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-green-500/50" placeholder="Nº" value={entrega.num} onChange={e => setEntrega({...entrega, num: e.target.value})} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-green-500/50" placeholder="Bairro" value={entrega.bairro} onChange={e => setEntrega({...entrega, bairro: e.target.value})} />
                    <input className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-green-500/50" placeholder="CEP" value={entrega.cep} onChange={e => setEntrega({...entrega, cep: e.target.value})} />
                  </div>
                </div>
              </div>

              <div className="mb-8 rounded-[2rem] border border-white/5 bg-slate-950/40 p-6 md:p-8">
                ...
                <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                  <select className="col-span-1 w-full cursor-pointer appearance-none rounded-2xl border border-white/10 bg-slate-950 p-5 text-[16px] font-bold text-white outline-none focus:border-cyan-500/50 md:col-span-3" value={vehicle} onChange={e => setVehicle(e.target.value as VehicleType)}>
                    {Object.entries(VEHICLE_CONFIG).map(([key, conf]) => (<option key={key} value={key}>{conf.nome}</option>))}
                  </select>
                  <input className="w-full rounded-2xl border border-white/10 bg-white/5 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="Peso (Ex: 200kg)" value={peso} onChange={e => setPeso(e.target.value)} />
                  <input className="w-full rounded-2xl border border-white/10 bg-white/5 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="Qtd Volumes (Ex: 4 Cx)" value={qtdVolumes} onChange={e => setQtdVolumes(e.target.value)} />
                  <input className="w-full rounded-2xl border border-white/10 bg-white/5 p-5 text-[16px] font-bold text-white outline-none placeholder:text-slate-600 focus:border-cyan-500/50" placeholder="O que é? (Móveis)" value={tipoMaterial} onChange={e => setTipoMaterial(e.target.value)} />
                </div>
                <div className="border-t border-white/5 pt-6">
                  ...
                  {tipoFrete === 'agendado' && <input type="datetime-local" className="mt-5 w-full max-w-md rounded-2xl border border-white/10 bg-slate-950 p-5 text-[16px] font-bold text-white outline-none focus:border-purple-500/50" value={dataAgendada} onChange={(e) => setDataAgendada(e.target.value)} />}
                </div>
              </div>
              ...
              {/* Botão de Calcular Frete no tamanho perfeito */}
              <button onClick={calcularDistanciaReal} disabled={loadingRoute || loadingPayment || !isFormValid} className={`flex w-full items-center justify-center gap-3 rounded-[1.5rem] py-5 text-[16px] font-black uppercase italic tracking-[0.2em] transition-all duration-300 ${!isFormValid ? 'cursor-not-allowed bg-slate-800 text-slate-600' : 'bg-cyan-500 text-slate-950 shadow-[0_15px_40px_rgba(6,182,212,0.35)] hover:scale-[1.02] hover:bg-cyan-400 active:scale-95'}`}>
                {loadingRoute ? <><Loader2 className="h-6 w-6 animate-spin"/> Mapeando Rota...</> : <><Zap size={22}/> Calcular Frete</>}
              </button>
            </div>
          )}
            ...
            <div className="w-full grid grid-cols-1 gap-8 animate-in fade-in zoom-in duration-500 lg:grid-cols-[1fr_420px]">
              ...
              <div className="relative flex h-full flex-col overflow-hidden rounded-[3rem] border border-cyan-500/20 bg-slate-900/90 p-8 lg:p-10 shadow-[0_20px_60px_rgba(6,182,212,0.15)] backdrop-blur-xl">
                ...
                <div className="mt-auto space-y-4">
                  <button onClick={handleContratar} disabled={loadingPayment || isProcessingPayment.current} className={`flex w-full items-center justify-center gap-3 rounded-[1.25rem] px-8 py-5 text-[14px] font-black uppercase tracking-[0.2em] shadow-[0_15px_40px_rgba(6,182,212,0.3)] transition-all hover:scale-[1.02] hover:bg-cyan-400 active:scale-95 ${loadingPayment ? 'bg-slate-800 text-slate-500' : 'bg-cyan-500 text-slate-950'}`}>{loadingPayment ? <><Loader2 className="h-6 w-6 animate-spin" /> Gerando Pagamento...</> : <><Zap size={20} /> Liberar Motorista</>}</button>
                  <button onClick={() => setStep('form')} className="flex w-full items-center justify-center rounded-[1.25rem] border border-white/10 bg-transparent px-8 py-5 text-xs font-black uppercase tracking-[0.2em] text-slate-400 transition-colors hover:bg-white/5 hover:text-white">Voltar e Editar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {step === 'busca' && (
            <div className="w-full grid grid-cols-1 gap-8 animate-in slide-in-from-bottom-6 duration-500 lg:grid-cols-[1fr_420px]">
              <div className="relative overflow-hidden rounded-[3rem] border border-white/10 bg-slate-900/80 p-6 md:p-10 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-xl">
                <div className="mb-8 flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
                  ...
                  <div className="flex items-center gap-3 self-start rounded-full border border-cyan-500/20 bg-cyan-500/10 px-5 py-2.5 shadow-inner sm:self-auto">
                    <div className="h-2.5 w-2.5 animate-ping rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,1)]" />
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  {orderData?.motoristaZap && <button onClick={handleWhatsAppClick} className="flex w-full items-center justify-center gap-3 rounded-[1.25rem] bg-green-500 px-6 py-5 text-[15px] font-black uppercase tracking-[0.2em] text-slate-950 shadow-[0_15px_35px_rgba(34,197,94,0.3)] transition-all hover:scale-[1.02] hover:bg-green-400 active:scale-95"><MessageCircle size={20} /> Chamar no WhatsApp</button>}
                  {(!orderData || ![TripState.ENTREGUE, TripState.CANCELADO].includes(orderData?.status as any)) && (
                    <button onClick={() => setShowCancelModal(true)} disabled={isCancelling} className="flex w-full items-center justify-center gap-2 rounded-[1.25rem] border border-white/5 bg-slate-900/80 px-6 py-5 text-[12px] font-black uppercase tracking-[0.2em] text-slate-500 backdrop-blur-md transition-all hover:border-red-500/30 hover:bg-red-500/10 hover:text-red-400"><XCircle size={18} /> Cancelar Operação</button>
                  )}
                </div>
              </div>
            </div>
