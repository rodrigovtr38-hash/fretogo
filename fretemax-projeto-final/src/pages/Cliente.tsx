  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col bg-[#020617] text-slate-200 font-sans selection:bg-cyan-500/30">
      ...
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        ...
        {/* Radar Gigante Seguro (Isolado e Ancorado) */}
      ...
      {/* MAIN CONTAINER (ENTERPRISE WIDTH & ORCHESTRATION) */}
      <main className="relative z-10 mx-auto flex w-full max-w-[1400px] flex-1 flex-col px-6 py-10 lg:px-8">
        
        {/* =====================================================
            STEP: FORMULÁRIO COMPLETO
        ===================================================== */}
        {step === 'form' && (
          <div className="mx-auto w-full max-w-4xl rounded-[2.5rem] border border-white/10 bg-slate-900/60 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 md:p-12">
            ...
            <div className="mb-10">
              ...
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <input
                  className="w-full rounded-2xl border border-white/10 bg-slate-950/50 p-5 text-sm font-bold text-white transition-all placeholder:text-slate-600 focus:border-cyan-500/50 outline-none"
            ...
            <div className="mb-10 rounded-[2rem] border border-white/5 bg-slate-950/40 p-8 shadow-inner">
              ...
              <div className="mb-6 grid grid-cols-1 gap-5 md:grid-cols-3">
                <select 
                  className="col-span-1 cursor-pointer appearance-none rounded-2xl border border-white/10 bg-slate-950 p-5 text-sm font-bold text-white outline-none focus:border-cyan-500/50 md:col-span-3 transition-colors"
                  value={vehicle} 
              ...
              <div className="border-t border-white/5 pt-6">
                <div className="mb-4 flex items-center gap-2">
                   ...
                   <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Horário da Coleta</p>
                </div>
                ...
                {tipoFrete === 'agendado' && (
                  <input type="datetime-local" className="mt-5 w-full max-w-md rounded-2xl border border-white/10 bg-slate-950 p-5 text-sm font-bold text-white outline-none focus:border-purple-500/50 transition-colors" value={dataAgendada} onChange={(e) => setDataAgendada(e.target.value)} />
                )}
              </div>
            </div>
        ...
        {/* =====================================================
            STEP: PREVIEW / RESUMO OPERACIONAL
        ===================================================== */}
        {step === 'preview' && (
          <div className="grid w-full grid-cols-1 gap-8 animate-in fade-in zoom-in duration-500 xl:grid-cols-[1fr_420px]">
            
            {/* Mapa Preview */}
            <div className="rounded-[3rem] border border-white/10 bg-slate-900/80 p-8 lg:p-10 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-xl">
              <div className="mb-10 flex items-center justify-between border-b border-white/5 pb-6">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.25em] text-cyan-400 font-bold">Prévia Operacional</p>
                  <h2 className="mt-2 text-4xl font-black italic tracking-tighter text-white">Trajeto Mapeado</h2>
                </div>
                <CheckCircle className="h-12 w-12 text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]" />
              </div>

              <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="rounded-3xl border border-white/5 bg-slate-950/60 p-6 shadow-inner hover:border-white/10 transition-colors">
                  ...
                  <p className="mt-2 text-base font-bold leading-snug text-white">{coleta.rua}, {coleta.num}</p>
                  <p className="mt-1 text-sm text-slate-400">{coleta.bairro}</p>
                </div>
                <div className="rounded-3xl border border-white/5 bg-slate-950/60 p-6 shadow-inner hover:border-white/10 transition-colors">
                  <Truck className="mb-4 h-6 w-6 text-green-400" />
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Veículo e Carga</p>
                  <p className="mt-2 text-base font-bold uppercase italic text-white">{VEHICLE_CONFIG[vehicle].nome}</p>
                  <p className="mt-1 text-sm text-slate-400">{peso} • {qtdVolumes} volumes</p>
                </div>
              </div>

              <div className="h-[400px] overflow-hidden rounded-[2.5rem] border border-white/10 bg-slate-950 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] md:h-[500px]">
                <MapaCliente />
              </div>
            </div>

            {/* SIDEBAR PREVIEW (Valor) */}
            <div className="relative flex h-full flex-col overflow-hidden rounded-[3rem] border border-cyan-500/20 bg-slate-900/90 p-8 lg:p-10 shadow-[0_20px_60px_rgba(6,182,212,0.15)] backdrop-blur-xl">
              ...
              <div className="mb-8 flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.25em] text-cyan-300">Valor Blindado</span>
              </div>

              <div className="mb-10">
                 <p className="text-sm font-bold text-slate-500 line-through mb-1">Médio: R$ {valorAncora.toFixed(2).replace('.', ',')}</p>
                 <h2 className="text-6xl font-black tracking-tighter text-white drop-shadow-md">R$ {valorTotalBruto.toFixed(2).replace('.', ',')}</h2>
              </div>
              ...
              <div className="mb-10 space-y-5 rounded-[2rem] border border-white/5 bg-slate-950/60 p-6 text-sm text-slate-300 shadow-inner">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  ...
                  <strong className="rounded-lg bg-slate-800 px-3 py-1.5 text-xs text-white">{validDistancia.toFixed(1)} km</strong>
                </div>
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <span className="font-bold">Modalidade</span>
                  <strong className="text-[10px] font-black uppercase tracking-widest text-cyan-400">{tipoFrete}</strong>
                </div>
                <div className="flex items-center justify-between">
                  <span className="font-bold">Material</span>
                  <strong className="max-w-[140px] truncate text-right text-white font-bold">{tipoMaterial || 'N/A'}</strong>
                </div>
              </div>
              ...
              <div className="mt-auto space-y-4">
                <button
                  ...
                  className={`flex min-h-[76px] w-full items-center justify-center gap-3 rounded-[1.5rem] px-8 py-5 text-[14px] font-black uppercase tracking-[0.2em] shadow-[0_15px_40px_rgba(6,182,212,0.3)] transition-all duration-300 ${loadingPayment ? 'bg-slate-800 text-slate-500' : 'bg-cyan-500 text-slate-950 hover:scale-[1.02] hover:bg-cyan-400 active:scale-95'}`}
                >
                  {loadingPayment ? <><Loader2 className="h-6 w-6 animate-spin" /> Gerando Pagamento...</> : <><Zap size={20} /> Liberar Motorista</>}
                </button>
                <button
                  onClick={() => setStep('form')}
                  className="flex min-h-[64px] w-full items-center justify-center rounded-[1.5rem] border border-white/10 bg-transparent px-8 py-4 text-xs font-black uppercase tracking-[0.2em] text-slate-400 transition-colors hover:bg-white/5 hover:text-white"
                >
                  Voltar e Editar
                </button>
              </div>
        ...
        {/* =====================================================
            STEP: BUSCA REALTIME (RADAR LOGÍSTICO VIVO)
        ===================================================== */}
        {step === 'busca' && (
          <div className="grid w-full grid-cols-1 gap-8 animate-in slide-in-from-bottom-6 duration-500 xl:grid-cols-[1fr_420px]">
            
            {/* MAPA E STATUS */}
            <div className="relative overflow-hidden rounded-[3rem] border border-white/10 bg-slate-900/80 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-xl lg:p-10">
              
              <div className="mb-8 flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
                <div>
                  <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.25em] text-cyan-400">
              ...
              {/* MAPA CONTAINER */}
              <div className="relative mb-8 h-[400px] overflow-hidden rounded-[2.5rem] border border-white/10 shadow-[inset_0_0_20px_rgba(0,0,0,0.5)] lg:h-[500px]">
                 {orderData?.status === 'disponivel' && (
                   <div className="pointer-events-none absolute inset-0 z-10 animate-pulse bg-cyan-500/10 mix-blend-overlay"></div>
                 )}
            ...
            <div className="space-y-8">
              ...
              {/* Card de Status */}
              <div className="rounded-[3rem] border border-cyan-500/20 bg-slate-900/90 p-10 shadow-[0_20px_50px_rgba(6,182,212,0.1)] backdrop-blur-xl relative overflow-hidden">
                ...
                {['aguardando_pagamento', 'disponivel', 'agendado'].includes(orderData?.status || '') ? (
                  <div className="py-8 text-center">
                    <div className="relative mx-auto mb-10 h-28 w-28">
                       ...
                       <div className="relative z-10 flex h-full w-full items-center justify-center rounded-full border border-cyan-500/30 bg-slate-950 shadow-[0_0_40px_rgba(6,182,212,0.25)]">
                         <Radar className="h-12 w-12 animate-spin text-cyan-400" style={{ animationDuration: '4s' }} />
                       </div>
                    </div>
                    <h3 className="mb-5 text-2xl font-black uppercase italic text-white tracking-tight">
                    ...
                    <div className="rounded-2xl border border-white/5 bg-slate-950/60 p-5 shadow-inner">
                      <p className="flex min-h-[40px] items-center justify-center text-xs font-bold uppercase tracking-widest text-cyan-400 leading-snug">
                  ...
                  <div>
                     <div className="mb-10 border-b border-white/5 pb-10 text-center">
                       <div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-[1.5rem] border border-blue-500/30 bg-blue-500/10 shadow-[0_0_20px_rgba(59,130,246,0.2)]">
                         <Truck className="h-10 w-10 text-blue-400 drop-shadow-md" />
                       </div>
                       <p className="mb-2 text-[10px] font-black uppercase tracking-[0.25em] text-slate-500">Motorista Confirmado</p>
                       <h3 className="text-3xl font-black uppercase italic text-white tracking-tighter">{orderData?.motoristaNome || 'Parceiro'}</h3>
                     </div>

                     <div className="space-y-10 pl-2">
                        <div className="relative pl-8">
                           <div className={`absolute -left-[6px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-slate-900 transition-all ${['aceito', 'coleta', 'em_transporte', 'entregue'].includes(orderData!.status) ? 'bg-cyan-400 shadow-[0_0_12px_rgba(34,211,238,0.8)] scale-110' : 'bg-slate-700'}`}></div>
                           <p className={`text-sm font-black uppercase tracking-widest ${['aceito', 'coleta', 'em_transporte', 'entregue'].includes(orderData!.status) ? 'text-cyan-400' : 'text-slate-500'}`}>Indo para o local</p>
                        </div>
                        <div className="relative pl-8">
                           <div className={`absolute -left-[0px] -top-8 h-8 w-0.5 ${['coleta', 'em_transporte', 'entregue'].includes(orderData!.status) ? 'bg-cyan-400/50' : 'bg-slate-800'}`}></div>
                           <div className={`absolute -left-[6px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-slate-900 transition-all ${['coleta', 'em_transporte', 'entregue'].includes(orderData!.status) ? 'bg-blue-400 shadow-[0_0_12px_rgba(59,130,246,0.8)] scale-110' : 'bg-slate-700'}`}></div>
                           <p className={`text-sm font-black uppercase tracking-widest ${['coleta', 'em_transporte', 'entregue'].includes(orderData!.status) ? 'text-blue-400' : 'text-slate-500'}`}>Embarcando Carga</p>
                        </div>
                        <div className="relative pl-8">
                           <div className={`absolute -left-[0px] -top-8 h-8 w-0.5 ${['em_transporte', 'entregue'].includes(orderData!.status) ? 'bg-blue-400/50' : 'bg-slate-800'}`}></div>
                           <div className={`absolute -left-[6px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-slate-900 transition-all ${['em_transporte', 'entregue'].includes(orderData!.status) ? 'bg-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.8)] scale-110' : 'bg-slate-700'}`}></div>
                           <p className={`text-sm font-black uppercase tracking-widest ${['em_transporte', 'entregue'].includes(orderData!.status) ? 'text-amber-400' : 'text-slate-500'}`}>Em Transporte</p>
                        </div>
                        <div className="relative pl-8">
                           <div className={`absolute -left-[0px] -top-8 h-8 w-0.5 ${['entregue'].includes(orderData!.status) ? 'bg-amber-400/50' : 'bg-slate-800'}`}></div>
                           <div className={`absolute -left-[6px] top-1.5 h-3.5 w-3.5 rounded-full border-2 border-slate-900 transition-all ${['entregue'].includes(orderData!.status) ? 'bg-green-400 shadow-[0_0_12px_rgba(74,222,128,0.8)] scale-110' : 'bg-slate-700'}`}></div>
                           <p className={`text-sm font-black uppercase tracking-widest ${['entregue'].includes(orderData!.status) ? 'text-green-400' : 'text-slate-500'}`}>Entregue</p>
                        </div>
                     </div>
                  </div>
                )}
              ...
              {/* Botões Ação Operacional */}
              <div className="space-y-4">
                {orderData?.motoristaZap && (
                  <button onClick={handleWhatsAppClick} className="flex min-h-[72px] w-full items-center justify-center gap-3 rounded-[1.5rem] bg-green-500 px-6 text-sm font-black uppercase tracking-[0.2em] text-slate-950 shadow-[0_15px_35px_rgba(34,197,94,0.3)] transition-all duration-300 hover:scale-[1.02] hover:bg-green-400 active:scale-95">
                ...
                {['aguardando_pagamento', 'disponivel', 'agendado'].includes(orderData?.status || '') && (
                  <button
                    ...
                    className="flex min-h-[64px] w-full items-center justify-center gap-2 rounded-[1.5rem] border border-white/5 bg-slate-900/80 px-6 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 backdrop-blur-md transition-all hover:border-red-500/30 hover:bg-red-500/10 hover:text-red-400"
                  >
