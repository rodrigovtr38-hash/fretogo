...
export default function Home() {
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#020617] text-white flex flex-col">
      ...
      {/* RADAR SYSTEM - O seu original que pulsa perfeitamente */}
      ...
      {/* WHATSAPP */}
      <a href="https://wa.me/5511946099840" target="_blank" rel="noreferrer" className="fixed bottom-6 right-6 z-[100] rounded-full bg-[#25D366] p-4 shadow-[0_10px_35px_rgba(37,211,102,0.4)] hover:bg-[#20bd5a] hover:scale-110 active:scale-95 transition-all duration-300 group">
        ...
        <section className="mx-auto grid w-full max-w-7xl grid-cols-1 items-center justify-items-center gap-16 px-6 py-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-8 lg:py-24">
          ...
          <div className="flex w-full max-w-[620px] flex-col justify-center lg:justify-self-start">
            ...
            <div className="mb-8 inline-flex w-fit items-center gap-3 rounded-full border border-cyan-500/20 bg-cyan-950/40 px-5 py-2.5 backdrop-blur-md shadow-[0_4px_25px_rgba(6,182,212,0.15)]">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-cyan-300 drop-shadow-sm">Radar ativo em sua região</span>
            </div>

            <h1 className="text-5xl font-black leading-[1.05] tracking-tight text-white md:text-6xl lg:text-[4.5rem]">
              SUA CARGA NO <span className="text-gradient-cyan italic drop-shadow-[0_0_35px_rgba(6,182,212,0.6)]">RADAR</span>.<br />
            ...
            {/* AQUI OS BOTÕES ESTÃO COM AS CORES CORRETAS */}
            <div className="mt-12 flex w-full flex-col gap-5 sm:flex-row sm:flex-wrap">
              <Link to="/contratar" className="group flex min-h-[76px] w-full flex-1 items-center justify-center gap-3 rounded-[1.25rem] bg-cyan-500 px-10 py-5 text-[15px] font-black uppercase italic tracking-widest text-slate-950 shadow-[0_15px_40px_rgba(6,182,212,0.4)] transition-all duration-300 hover:scale-[1.02] hover:bg-cyan-400 active:scale-95 sm:w-auto">
                Contratar Frete
                <ChevronRight size={20} className="transition-transform duration-300 group-hover:translate-x-1.5" />
              </Link>

              <Link to="/parceiros" className="flex min-h-[76px] w-full flex-1 items-center justify-center rounded-[1.25rem] bg-cyan-500 px-10 py-5 text-[15px] font-black uppercase italic tracking-widest text-slate-950 shadow-[0_15px_40px_rgba(6,182,212,0.4)] transition-all duration-300 hover:scale-[1.02] hover:bg-cyan-400 active:scale-95 sm:w-auto">
                Ser Motorista
              </Link>

              <button onClick={() => setShowModal(true)} className="flex min-h-[76px] w-full items-center justify-center gap-3 rounded-[1.25rem] bg-white px-10 py-5 text-[15px] font-black uppercase italic tracking-widest text-black shadow-[0_15px_35px_rgba(255,255,255,0.2)] transition-all duration-300 hover:scale-[1.02] hover:bg-slate-200 active:scale-95 sm:w-auto">
      ...
      {/* BENEFITS SECTION - O RODAPÉ DO SEU CÓDIGO ORIGINAL */}
