...
export type DispatchOperationalState = 'OFFLINE' | 'ONLINE' | 'MATCHING' | 'RESERVED' | 'ACTIVE_TRIP' | 'COLLECTING' | 'IN_TRANSIT' | 'DELIVERING' | 'RETURNING';
export type TrackingOperationalState = 'IDLE' | 'ACTIVE' | 'COLLECTING' | 'MOVING' | 'DELIVERING' | 'RETURNING';
export type AvailabilityOperationalState = 'AVAILABLE' | 'RESERVED' | 'BUSY' | 'RETURNING' | 'OFFLINE';
...
export class StateSynchronizationService {
  static synchronize(driverState: DriverState, tripState: AppTripState): OperationalSyncResult {
    ...
    switch (tripState) {
      ...
      case AppTripState.DISPONIVEL:
        return {
          driverState: DriverState.ONLINE, tripState, dispatchState: 'MATCHING', trackingState: 'IDLE', availabilityState: 'AVAILABLE',
      ...
      case AppTripState.ACEITO:
        return {
          driverState: DriverState.ACEITOU, tripState, dispatchState: 'ACTIVE_TRIP', trackingState: 'ACTIVE', availabilityState: 'BUSY',
      ...
      case AppTripState.INDO_COLETA:
        return {
          driverState: DriverState.INDO_COLETA, tripState, dispatchState: 'COLLECTING', trackingState: 'COLLECTING', availabilityState: 'BUSY',
          operationalRuntime: { matching: false, tracking: true, radar: false, dispatch: true, returning: false, realtime: true },
        };

      case AppTripState.COLETANDO:
        return {
          driverState: DriverState.COLETANDO, tripState, dispatchState: 'COLLECTING', trackingState: 'COLLECTING', availabilityState: 'BUSY',
          operationalRuntime: { matching: false, tracking: true, radar: false, dispatch: true, returning: false, realtime: true },
        };

      case AppTripState.EM_TRANSPORTE:
        return {
          driverState: DriverState.EM_TRANSPORTE, tripState, dispatchState: 'IN_TRANSIT', trackingState: 'MOVING', availabilityState: 'BUSY',
          operationalRuntime: { matching: false, tracking: true, radar: false, dispatch: true, returning: false, realtime: true },
        };

      case AppTripState.FINALIZANDO:
        return {
          driverState: DriverState.FINALIZANDO, tripState, dispatchState: 'DELIVERING', trackingState: 'DELIVERING', availabilityState: 'BUSY',
          operationalRuntime: { matching: false, tracking: true, radar: false, dispatch: true, returning: false, realtime: true },
        };

      case AppTripState.ENTREGUE:
        return {
          driverState: DriverState.ENTREGUE, tripState, dispatchState: 'RETURNING', trackingState: 'RETURNING', availabilityState: 'RETURNING',
      ...
      case AppTripState.ERRO_PAGAMENTO:
        return {
          driverState: DriverState.ONLINE, tripState, dispatchState: 'ONLINE', trackingState: 'IDLE', availabilityState: 'AVAILABLE',
          operationalRuntime: { matching: true, tracking: false, radar: true, dispatch: true, returning: false, realtime: true },
        };

      default:
        return {
          driverState, tripState, dispatchState: 'ONLINE', trackingState: 'IDLE', availabilityState: 'AVAILABLE',
          operationalRuntime: { matching: true, tracking: false, radar: true, dispatch: true, returning: false, realtime: true },
        };
    }
  }
