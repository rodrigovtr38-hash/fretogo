...
class LocationService {
  private readonly fallbackRegions: Record<number, Coordinates> = {
    ...
    5: { lat: -3.7319, lng: -38.5267 },  // CE
    6: { lat: -15.7797, lng: -47.9297 }, // DF
    7: { lat: -25.4284, lng: -49.2733 }, // PR
    8: { lat: -30.0346, lng: -51.2177 }, // RS
    9: { lat: -16.6869, lng: -49.2648 }, // GO
  };

  private readonly cepCache = new Map<string, Coordinates>();

  /* =======================================================
     CEP LOOKUP
  ======================================================= */
  async buscarEnderecoPorCEP(cep: string, signal?: AbortSignal): Promise<ViaCepResponse | null> {
    try {
      ...
      console.error('[CTO-Log] ❌ ERRO CEP:', error);
      return null;
    }
  }
  ...
  async getCurrentLocation(): Promise<Coordinates | null> {
    ...
    if (!navigator.geolocation) {
      console.warn('[CTO-Log] ⚠️ Geolocation indisponível.');
      return null;
    }
    ...
    return new Promise(resolve => {
      navigator.geolocation.getCurrentPosition(
        ...
        error => {
          console.error('[CTO-Log] ❌ GEOLOCATION ERROR:', error);
          resolve(null);
        },
  ...
  /* =======================================================
     CEP TO COORDINATES
  ======================================================= */
  async getCoordinatesFromCEP(cep: string): Promise<Coordinates | null> {
    try {
      const sanitizedCep = cep.replace(/\D/g, '');

      if (this.cepCache.has(sanitizedCep)) {
        return this.cepCache.get(sanitizedCep) || null;
      }

      const address = await this.buscarEnderecoPorCEP(sanitizedCep);
      ...
      console.error('[CTO-Log] ❌ CEP COORDINATES ERROR:', error);
      return null;
    }
  }
  ...
  /* =======================================================
     FALLBACK COORDS
  ======================================================= */
  getFallbackCoords(cep: string): Coordinates {
    const prefix = parseInt(cep.replace(/\D/g, '').substring(0, 1) || '0', 10);
    return this.fallbackRegions[prefix] || this.fallbackRegions[0];
  }

  /* =======================================================
     DISTANCE (HAVERSINE FORMULA)
  ======================================================= */
  calcularDistanciaKm(origem: Coordinates, destino: Coordinates): number {
    const R = 6371;
    const dLat = ((destino.lat - origem.lat) * Math.PI) / 180;
    const dLon = ((destino.lng - origem.lng) * Math.PI) / 180;
    const a =
  ...
  /* =======================================================
     VALIDATE COORDS
  ======================================================= */
  validarCoordenadas(coords?: Coordinates): boolean {
    if (!coords) return false;
    if (typeof coords.lat !== 'number' || typeof coords.lng !== 'number') return false;
    if (Number.isNaN(coords.lat) || Number.isNaN(coords.lng)) return false;
    return true;
  }
}
